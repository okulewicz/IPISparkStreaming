...
public final class SocketReceiver {
	...
	public static void main(String[] args) {
		...
		JavaPairDStream<String, Set<Long>> stream = lines.flatMap(line -> Lists.newArrayList(SEPARATOR.split(line)))
				.filter(word -> {
					try {
						Long.valueOf(word);
					} catch (NumberFormatException ex) {
		...
		stream.foreachRDD(new Function<JavaPairRDD<String, Set<Long>>, Void>() {
			@Override
		...
		stream.checkpoint(CHECKPOINT_INTERVAL);
